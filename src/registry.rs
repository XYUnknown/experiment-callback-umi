...
macro_rules! register {
    ...
    //four args - return ref
    ($map:ident, $name:ident, $fn_name:path, $fn_ty:ty, ($res_ty_plain:ty, $res_ty:ty, ResultOp::Ref), $args_ty_plain1:ty, $args_ty_plain2:ty, $args_ty_plain3:ty, $args_ty_plain4:ty, $args_ty1:ty, $args_ty2:ty, $args_ty3:ty, $args_ty4:ty) => {
        #[derive(Clone)]
        pub struct $name {
            ptr: fn($args_ty1, $args_ty2, $args_ty3, $args_ty4) -> $res_ty,
        }

        impl GenCall for $name {
            fn call(&self, mut args: Vec<Argument>) -> ((String, bool), Box<dyn Any + Send + Sync>){
                let mut arg4 = args.pop().unwrap();
                let mut arg3 = args.pop().unwrap();
                let mut arg2 = args.pop().unwrap();
                let mut arg1 = args.pop().unwrap();
                let w1 = arg1.get_arg::<$args_ty_plain1>();
                let w2 = arg2.get_arg::<$args_ty_plain2>();
                let w3 = arg3.get_arg::<$args_ty_plain3>();
                let w4 = arg4.get_arg::<$args_ty_plain4>();
                let result = (self.ptr)(w1.extract(), w2.extract(), w3.extract(), w4.extract());
                let serialised = result.tagged_string();
                let ptr: *const $res_ty_plain = result;
                let boxed: Box<dyn Any + Send + Sync> = Box::new(ConstPtr(ptr));
                return (serialised, boxed);
    ...
    //four args - return mut ref
    ($map:ident, $name:ident, $fn_name:path, $fn_ty:ty, ($res_ty_plain:ty, $res_ty:ty, ResultOp::MutRef), $args_ty_plain1:ty, $args_ty_plain2:ty, $args_ty_plain3:ty, $args_ty_plain4:ty, $args_ty1:ty, $args_ty2:ty, $args_ty3:ty, $args_ty4:ty) => {
        #[derive(Clone)]
        pub struct $name {
            ptr: fn($args_ty1, $args_ty2, $args_ty3, $args_ty4) -> $res_ty,
        }
    ...
    // four args - retrurn owned
    ($map:ident, $name:ident, $fn_name:path, $fn_ty:ty, ($res_ty:ty, ResultOp::Owned), $args_ty_plain1:ty, $args_ty_plain2:ty, $args_ty_plain3:ty, $args_ty_plain4:ty, $args_ty1:ty, $args_ty2:ty, $args_ty3:ty, $args_ty4:ty) => {
        #[derive(Clone)]
        pub struct $name {
            ptr: fn($args_ty1, $args_ty2, $args_ty3, $args_ty4) -> $res_ty,
        }
